...
bot.on('message', async (msg) => {
  ...
  if (userTemp[chatId] && userTemp[chatId].action === 'awaiting_phone') {
    ...
    let phone = text.replace(/[^0-9]/g, ''); // keep only digits
    if (!phone || phone.length < 10) {
      await bot.sendMessage(chatId, '❌ Invalid phone number. Please enter a valid number with country code (e.g., 919876543210).');
      return;
    }
    ...
    try {
      ...
      await bot.editMessageText(`❌ Failed to add account: ${err.message}`, { chat_id: chatId, message_id: processingMsg.message_id });
    }
    // Clean up temporary state and show main menu
